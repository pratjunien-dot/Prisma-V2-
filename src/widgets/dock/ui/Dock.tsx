...
import { Home, MessageSquare, Users, Swords, Settings, Eye, EyeOff } from "lucide-react";
    ...
    <div className="fixed bottom-0 left-0 right-0 z-[100] flex flex-col items-center pb-[env(safe-area-inset-bottom,1.5rem)] pointer-events-none">
      ...
      <AnimatePresence>
        {isUiVisible && (
          ...
          >
            <Glass level={2} className="flex items-center gap-1 sm:gap-2 px-3 py-2 rounded-[32px] border-white/10 shadow-2xl backdrop-blur-3xl">
              {TABS.map((tab) => {
                const Icon = tab.icon;
